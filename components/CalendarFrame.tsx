...
const Arrow = ({fill, svgProps}: ArrowProps) => (
  ...
  >
    <path
      d="M6.4 35 24 17.497 6.4 0 0 6.363l11.2 11.134L0 28.637 6.4 35Z"
      fill={fill}
    />
  </svg>
)
    ...
    [
      {
        time: '5:30',
        name: "Event",
        desc: 'ahaha yep it\s saturday my dudes',
      },
        ...
        >
          ...
          <div className="h-24 w-full">
            <div className="bg-white/50 p-4 inline-flex justify-center items-center">
              <Arrow fill={day == 0 ? "#0000005C" : "#000"} svgProps={{className: "inline-block h-6 rotate-180", onClick: () => setDay(Math.min(day-1, sections.length-1))}} />
              <span className="text-3xl mx-4">{weekDays[day]}</span>
              <Arrow fill={day == sections.length - 1 ? "#0000005C" : "#000"} svgProps={{className: "inline-block h-6", onClick: () => setDay(Math.min(day+1, sections.length-1))}} />
          ...
          >
            {sections[day].map((val, ind) => (
              <div key={ind} className={"calendar-date w-full p-4 flex flex-col" + (Math.floor(ind / 7) < Math.floor(sections[day].length / 7) ? " has-bottom" : "")}>
                {/* time */}
